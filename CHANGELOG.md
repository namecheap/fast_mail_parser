...
- **`parse_email(payload, mode="lazy")`** decodes the bodies as usual and defers
  ...
  - **The trade, stated plainly: memory for decoding.** The encoded bytes of every
    ...
    attachment-heavy fixture, median of three interleaved rounds: metadata
    0.37 ms, lazy with nothing read 0.38 ms, full 1.08 ms, lazy with every
    attachment read 1.09 ms. Deferring saves ~65% when you were not going to
    decode everything and costs ~1% when you were. **Use the default mode if you
    are going to read every attachment.**
